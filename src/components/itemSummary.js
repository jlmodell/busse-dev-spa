...
const ItemSummary = props => {
  ...
  const FetchData = async () => {
    try {
      await axios
        .get(
          `${process.env.REACT_APP_itemSummaryApiUrl}?start=${
            state.start
          }&end=${state.end}&item=${props.name.split("|")[0]}`
        )
        .then(res => setItemSummary(res.data));
    } catch (err) {
  ...
  return (
    <div className="cards-modal">
      ...
      <div className="card-title">{props.name.split("|")[1]} - Summary</div>
